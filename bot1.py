...
# --- Configuration ---
TOKEN = "8846458271:AAE4FFjYENEZ16qwWowrvecttcFQ6voPrHs" #bot token dalo yeha
OWNER_ID = 8623320269 #yha tumhra chat id dalo
ADMIN_ID = 8623320269 #yeha koi admin ya tumhara chat id dalo
YOUR_USERNAME = '@aaryanown' #yeha tumhra username dala
UPDATE_CHANNEL = 'https://t.me/baunamon' #yeha chnl link dalo''
FORCE_JOIN_CHANNELS = {
...
# --- Small Caps Font System ---
# ALL user-facing bot text and keyboard button labels are converted to
# small-caps automatically. Callback data, URLs, filenames and user input
# are NOT modified.

import functools

_font_state = threading.local()
...
def _small_caps_keyboard_text(text):
    return to_small_caps(text) if isinstance(text, str) else text

# Patch Telegram output methods so every bot-generated visible message
# automatically uses small caps.
...
def _patched_send_message(chat_id, text, *args, **kwargs):
    return _original_send_message(
        chat_id, to_small_caps(text), *args, **kwargs
    )

def _patched_reply_to(message, text, *args, **kwargs):
    return _original_reply_to(
        message, to_small_caps(text), *args, **kwargs
    )

def _patched_edit_message_text(chat_id, message_id, text, *args, **kwargs):
    return _original_edit_message_text(
        chat_id, message_id, to_small_caps(text), *args, **kwargs
    )

def _patched_answer_callback_query(callback_query_id, text=None, *args, **kwargs):
    return _original_answer_callback_query(
        callback_query_id,
        to_small_caps(text) if text is not None else text,
        *args,
        **kwargs
    )
...
bot.answer_callback_query = _patched_answer_callback_query
bot.send_photo = _patched_send_photo
bot.send_document = _patched_send_document

# Patch keyboard button constructors. The displayed label becomes small
# caps while callback_data / URL values remain unchanged.
_original_keyboard_button = types.KeyboardButton
_original_inline_button = types.InlineKeyboardButton

def _patched_keyboard_button(text, *args, **kwargs):
    return _original_keyboard_button(
        _small_caps_keyboard_text(text), *args, **kwargs
    )

def _patched_inline_button(text, *args, **kwargs):
    return _original_inline_button(
        _small_caps_keyboard_text(text), *args, **kwargs
    )

types.KeyboardButton = _patched_keyboard_button
types.InlineKeyboardButton = _patched_inline_button

def normal_font(func):
    # Kept for compatibility with existing decorators.
    # User-facing command responses are now also small caps.
...
MALWARE_SIGNATURES = [
    ...
    # ZIP/RAR archives are handled separately by the archive checks below.
    b'Rar!',  # RAR archive
]

# Do not treat generic cryptography/tool names as malware indicators.
# Words such as "openssl", "RSA", "GPG", etc. can legitimately appear
# in normal Python/JS projects and cause false-positive security alerts.
...
def create_admin_panel():
    ...
    markup.row(types.InlineKeyboardButton('🔑 Change Bot Token', callback_data='change_token'))
...
def _logic_send_welcome(message):
    ...
    welcome_msg_text = (
        f"✨━━━━━━━━━━━━━━━━━━━✨\n"
        f"   🤖 *ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ ʜᴏsᴛɪɴɢ ʙᴏᴛ* 🤖\n"
        f"✨━━━━━━━━━━━━━━━━━━━✨\n\n"
        f"👋 Hey *{user_name}*, glad to have you here!\n\n"
        f"🆔 *User ID:* `{user_id}`\n"
        ...
        f"📦 Upload single files or `.zip` archives\n"
        f"━━━━━━━━━━━━━━━━━━━\n\n"
...
@bot.message_handler(func=lambda message: (
    message.text in BUTTON_TEXT_TO_LOGIC
    or any(to_small_caps(k) == message.text for k in BUTTON_TEXT_TO_LOGIC)
))
def handle_button_text(message):
    logic_func = BUTTON_TEXT_TO_LOGIC.get(message.text)
    if logic_func is None:
        for button_text, handler in BUTTON_TEXT_TO_LOGIC.items():
            if to_small_caps(button_text) == message.text:
                logic_func = handler
                break
    if logic_func:
        logic_func(message)
...
def handle_callbacks(call):
    ...
    if bot_locked and user_id not in admin_ids and data not in ['back_to_main', 'speed', 'stats', 'check_files']:
    ...
    try:
        ...
        elif data == 'change_token': owner_required_callback(call, change_token_init_callback)
...
def check_files_callback(call):
    """Show the caller's files reliably, including after a restart/redeploy."""
    user_id = call.from_user.id
    chat_id = call.message.chat.id

    # Refresh this user's file list from SQLite so the button does not depend
    # only on the in-memory dictionary surviving a process restart.
    try:
        with DB_LOCK:
            conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
            cur = conn.cursor()
            cur.execute(
                "SELECT file_name, file_type FROM user_files WHERE user_id = ? ORDER BY file_name",
                (user_id,)
            )
            db_files = cur.fetchall()
            conn.close()
        user_files[user_id] = [(name, ftype) for name, ftype in db_files]
    except Exception as e:
        logger.error(f"Error loading files for user {user_id}: {e}", exc_info=True)
    ...
    if not user_files_list:
        ...
        try:
            ...
            logger.error(f"Unexpected error editing empty file list: {e}", exc_info=True)
        return

    for file_name, file_type in sorted(user_files_list, key=lambda x: x[0].lower()):
        try:
            is_running = is_bot_running(user_id, file_name)
        except Exception:
            is_running = False
...
# --- Token Change System (Owner Only) ---
def _write_new_token_to_file(new_token):
    file_path = os.path.abspath(__file__)
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    updated_content, count = re.subn(r'TOKEN = "[^"]*"', f'TOKEN = "{new_token}"', content, count=1)
    if count == 0:
        raise ValueError("TOKEN line not found in source file.")
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(updated_content)

def _restart_bot_process():
    logger.warning("🔄 Restarting bot process to apply new token...")
    python_exe = sys.executable
    os.execv(python_exe, [python_exe] + sys.argv)

def change_token_init_callback(call):
    bot.answer_callback_query(call.id)
    msg = bot.send_message(call.message.chat.id,
                            "🔑 Naya bot token bhejo (BotFather se liya hua).\n/cancel to abort.")
    bot.register_next_step_handler(msg, process_new_token)

def process_new_token(message):
    if message.from_user.id != OWNER_ID:
        bot.reply_to(message, "⚠️ Owner only action."); return
    if message.text.strip().lower() == '/cancel':
        bot.reply_to(message, "Token change cancelled."); return
    new_token = message.text.strip()
    if not re.match(r'^\d{6,}:[A-Za-z0-9_-]{30,}$', new_token):
        bot.reply_to(message, "⚠️ Yeh valid bot token nahi lag raha.\nFormat: `123456789:ABCDefGhIjKlmNoPqRsTuVwXyZ...`\n\nDobara try karo ya /cancel bhejo.", parse_mode='Markdown')
        msg = bot.send_message(message.chat.id, "🔑 Naya token bhejo ya /cancel.")
        bot.register_next_step_handler(msg, process_new_token)
        return
    try:
        _write_new_token_to_file(new_token)
        logger.warning(f"🔑 Bot token changed by Owner {message.from_user.id}.")
        bot.reply_to(message, "✅ Token update ho gaya! Bot naye token ke saath restart ho raha hai...")
        threading.Timer(1.5, _restart_bot_process).start()
    except Exception as e:
        logger.error(f"Failed to change token: {e}", exc_info=True)
        bot.reply_to(message, f"❌ Token change fail ho gaya: {e}")

@bot.message_handler(commands=['chng'])
@normal_font
def command_change_token(message):
    if message.from_user.id != OWNER_ID:
        bot.reply_to(message, "⚠️ Yeh command sirf Owner use kar sakta hai."); return
    msg = bot.send_message(message.chat.id, "🔑 Naya bot token bhejo (BotFather se liya hua).\n/cancel to abort.")
    bot.register_next_step_handler(msg, process_new_token)
