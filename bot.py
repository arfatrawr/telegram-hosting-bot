...
# Commands (like /start) reply in NORMAL font.
# Everything else (button clicks, general replies) reply in ᴀʙᴄᴅ small-caps font.
import functools
...
def normal_font(func):
    """Decorator: mark a command handler so its replies stay in normal font."""
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        prev = getattr(_font_state, "normal", False)
        _font_state.normal = True
        try:
            return func(*args, **kwargs)
        finally:
            _font_state.normal = prev
    return wrapper
...
def _patched_send_message(chat_id, text, *args, **kwargs):
    if not getattr(_font_state, "normal", False):
